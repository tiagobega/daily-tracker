...
npm run build        # Production build (self-contained Nitro Node server in dist/)
npm run preview      # Preview the production build
npm run test         # Run Vitest once (vitest run)

npm run check        # Biome: lint + format + organize imports (the combined gate)
...
Run a single test: `npx vitest run path/to/file.test.ts` (or `npx vitest -t "test name"`).

Serve the production build: `node dist/server/index.mjs` after `npm run build`.

### Database (Drizzle + Postgres)

```bash
npm run db:push      # Push schema to DB (fastest for dev iteration)
...
**File-based routing:** routes are files in `src/routes/`. `src/routeTree.gen.ts` is auto-generated — never edit it by hand; it regenerates on dev/build and via `npm run generate-routes`. It's excluded from Biome.
...
- **Formatting (Biome, enforced):** tab indentation, double quotes. Run `npm run check` before finishing; it also auto-organizes imports. Note the app source in `src/` mostly uses 2-space/single-quote template style — Biome's config (tabs/double-quote) is the source of truth and will reformat on `check`.
